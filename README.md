...
[![Huggingface](https://img.shields.io/badge/Hugging_face-2306.09341-yellow)](https://huggingface.co/spaces/xswu/HPSv2)
...
Functions including the evaluation and inference of our preference model and reproducing the benchmark are packed up to a single python package [`hpsv2`](https://pypi.org/project/hpsv2/).
...
You can run the following code to prepare the images for evaluation:
...
for style, prompts in all_prompts.items():
    for prompt in prompts:
        ...
        # <image_path> is the input of hpsv2.evaluate()

```

To evaluate your own text-to-image generative model, you can generate test data base on the [benchmark prompts](https://huggingface.co/datasets/zhwang/HPDv2/tree/main/benchmark) we provide, and then run the following code:
...
We also provide [data](https://huggingface.co/datasets/zhwang/HPDv2/tree/main/benchmark/benchmark_imgs) generated by some of the major text-to-image generative models to set up [benchmarks](#the-hps-v2-benchmark). You can quickly download the data and evaluate the models by running the following code.
...
You can scoring single generated image and corresponding prompt by running the following code:

```python
import hpsv2

result = hpsv2.score('<img_path>', '<prompt>') 
```

Before evaluating and scoring, benchmark data and checkpoint will be downloaded automatically. The default download path is `~/.cache/hpsv2/`. You can change the download path by setting the environment variable `HPS_ROOT`.
